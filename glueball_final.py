...
# ================== CONSTANTES BSFT ==================
m_rho = 0.775  # GeV
Delta_AdS = 1.371  # Dimensão conforme AdS_5 puro
...
# ================== FUNÇÕES BSFT ==================
def bsft_mass(Delta):
    """Calcula massa BSFT dado Delta"""
    ratio = (1 + np.sqrt(Delta * (Delta + 4) / 4.0)) / (1 + np.sqrt(5/4))
    return ratio * m_rho

def wavefunction(r, Delta, is_3d=False):
    """Função de onda AdS vs 3D"""
    if not is_3d:
        # AdS puro: decaimento lento
        psi = r**2 * np.exp(-m_rho * r * 1.5)
    else:
        # 3D: confinamento pelos graus perp
...
sigma_diff = abs(M_3D - M_LATTICE)
sigma_total = np.sqrt(M_LATTICE_ERR**2 + M_3D_err**2)
sigma_agreement = sigma_diff / sigma_total
...
ax1.text(0.05, 0.90, f'Delta = {Delta_AdS:.3f}', transform=ax1.transAxes, fontsize=11)
...
# PAINEL 2: 3D Modulado com Banda de Erro
...
ax2.set_title(f'3D Physical: M = {M_3D:.2f} +/- {M_3D_err:.2f} GeV', 
...
ax2.text(0.05, 0.76, f'{sigma_agreement:.1f} sigma vs Lattice QCD', 
...
plt.suptitle('BSFT Scalar Glueball: Holographic 3D Modulation with Uncertainty', 
...
# ================== OUTPUT TERMINAL ==================
print("="*75)
print("BSFT Scalar Glueball Result v1.1.3 - Complete with Error Bars:")
print("="*75)
print(f"AdS_5 Pure : Delta = {Delta_AdS:.3f} -> M = {M_AdS:.3f} GeV")
...
print(f"Agreement   : {sigma_agreement:.1f} sigma")
print("-"*75)
print("Physical Interpretation:")
print(f"Delta_3D = Delta_AdS + 3 * C_BSFT")
print(f"         = {Delta_AdS:.3f} + 3 * {C_BSFT:.3f} = {Delta_3D:.2f}")
